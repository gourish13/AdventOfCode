...
func sizeWillBeDeleted() int {
    var freeSpace = reqdSpace - (totalSpace - dirs[0].size)

    var filteredDirs = []*Dir{}
    for _, dir := range dirs {
        if dir.size >= freeSpace {
            filteredDirs = append(filteredDirs, dir)
        }
    }

    var minSpace = totalSpace
